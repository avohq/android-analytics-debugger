// Generated by Avo VERSION 52.21.0, PLEASE EDIT WITH CARE
package sh.avo
...
private object AvoLogger {
    fun logEventSent(
        ...
        Log.d(this.javaClass.simpleName,"[avo] Event Sent: $eventName Event Props: $eventProperties User Props: $userProperties")
    }
}
    ...
    fun invoke(eventId: String, hash: String, messages: List<AvoAssertMessage>) {
        if (samplingRate > 0) {
            if (Math.random() < samplingRate) {
                val json = JSONObject()
                json.put("ac", "TlAnsHYx57GLSNB3fbF0")
                json.put("br", "master")
                json.put("en", "dev")
                json.put("ev", eventId)
                json.put("ha", hash)
